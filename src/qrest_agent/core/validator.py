...
from qrest_agent.core.schema import CHANNEL_REQUIRED_KEYS, QREST_REQUIRED_PATHS
...
def validate_state(state: MetadataState) -> ValidationReport:
    ...
    for path in QREST_REQUIRED_PATHS:
        ...
        if record is not None and record.status == "conflict":
            ...
            missing.append(path)
            issues.append(ValidationIssue("error", path, "required qREST metadata field is missing"))

    _validate_root(metadata, issues)
    _validate_building(metadata, issues)
    _validate_instrument(metadata, issues)
    _validate_data_info(metadata, issues)

    ready = not missing and not conflicts and not any(issue.level == "error" for issue in issues)
    return ValidationReport(ready=ready, missing_required=missing, conflicts=conflicts, issues=issues)
...
def _validate_instrument(metadata: dict[str, Any], issues: list[ValidationIssue]) -> None:
    ...
    for index, channel in enumerate(channels):
        ...
        for key in CHANNEL_REQUIRED_KEYS:
            if key not in channel:
                issues.append(ValidationIssue("error", f"{path}.{key}", "required channel field is missing"))

        channel_no = channel.get("ChannelNo")
        if isinstance(channel_no, int):
        ...
        if not _is_number_list(location, 3, int | float):
